...
unsigned int LowWrData;
unsigned int UppWrData;
				...
				case DATA_RECORD:  //Record Type 00, data record.
					...
					while(HexRecordSt.RecDataLen) // Loop till all bytes are done.
					{
						   ...
						   )//&& (ProgAddress < (void*)DEV_CONFIG_REG_BASE_ADDRESS) )//|| (ProgAddress > (void*)DEV_CONFIG_REG_END_ADDRESS)))
						{
                                ...
                                // If data length is less than 8
                             if(HexRecordSt.RecDataLen < 8)
                             {
                                 // If data length is equal or less than 4 (we have to write only one word)
                                 if (HexRecordSt.RecDataLen <= 4)
                                 {
                                     // Appending 0xFF will make sure that we don't write junk data
                                     LowWrData = 0xFFFFFFFF;
                                     UppWrData = 0xFFFFFFFF;
                                     memcpy(&LowWrData, HexRecordSt.Data, HexRecordSt.RecDataLen);
                                 }
                                 else
                                 { // If data length is greater than 4 (we have to write two words)
                                     // Appending 0xFF will make sure that we don't write junk data
                                     LowWrData = 0xFFFFFFFF;
                                     UppWrData = 0xFFFFFFFF;
                                     // Lower Word
                                     memcpy(&LowWrData, HexRecordSt.Data, 4);
                                     // Increase pointer
                                     HexRecordSt.Data += 4;
                                     // Upper Word
                                     memcpy(&UppWrData, HexRecordSt.Data, (HexRecordSt.RecDataLen - 4));
                                 }
                             }
                             else
                             {
                                 // If data length is greater or equal to 8 (we have to write two full words)
                                 // Write lower word
                                 memcpy(&LowWrData, HexRecordSt.Data, 4);
                                 // Increment pointer to next data
                                 HexRecordSt.Data +=4;
                                 // Write upper word
                                 memcpy(&UppWrData, HexRecordSt.Data, 4);
                             }

                             // Write the data into flash.
                             Result = NVMemWriteWord(ProgAddress, LowWrData, UppWrData);
