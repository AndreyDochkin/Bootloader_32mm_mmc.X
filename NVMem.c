...
const uint32_t countPerMicroSec = ((F_CPU/1000000)/2);
...
UINT __attribute__((nomips16)) NVMemOperation(UINT nvmop)
{
    ...
    NVMCONCLR = 7;//clear errors WRERR LVDERR
    asm("nop");asm("nop");

    // Enable Flash Write/Erase Operations
   // NVMCON = NVMCON_WREN | nvmop;
   NVMCON = nvmop; // set operation
    //NVMCON = NVMCON | NVMCON_WREN;//enable WR bit
 ...
 ********************************************************************/
unsigned int NVMemWriteWord(void* address, unsigned int dLow, unsigned int dHigh)
{
    unsigned int res;
 ...
 *                  a single operation is 1 row 64 instructions (256 Bytes).  The row at
